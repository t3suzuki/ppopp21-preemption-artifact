...
int ABTI_xstream_create(ABTI_sched *p_sched, ABTI_xstream **pp_xstream)
{
    ...
    p_newxstream->disable_sleep      = ABT_FALSE;
    p_newxstream->should_sleep       = ABT_FALSE;
    p_newxstream->sleep_flag         = 0;
...
int ABTI_xstream_start(ABTI_xstream *p_xstream)
{
    ...
    if (p_xstream->type == ABTI_XSTREAM_TYPE_PRIMARY) {
        ...
        ABTI_sub_xstream_list_local_init(&p_xstream->sub_xstream_list,
                                         init_num_sub_xstreams);

        /* Create a timer */
        ABTI_preemption_timer_create(p_xstream);
...
int ABT_xstream_join(ABT_xstream xstream)
{
  ...
  fn_join:
    /* Return to the main ES. This is required because the current sub ES can
     * be the target's main ES. */
...
int ABT_xstream_get_active_num(int *num_xstreams)
{
    ABTI_ENTER;
    int abt_errno = ABT_SUCCESS;
    ...
    *num_xstreams = gp_ABTI_global->num_active_xstreams;
...
int ABTI_xstream_check_events(ABTI_xstream *p_xstream, ABT_sched sched)
{
    ...
    /* Periodically check whether the current sub ES is the main ES of another
     * ES. If so, switch to another sub ES. */
    ABTI_sched *p_sched = ABTI_sched_get_ptr(sched);
    ABTI_sub_xstream_exit_main(p_sched->p_thread);
    ...
    if (p_xstream->request & ABTI_XSTREAM_REQ_FREE_TIMER) {
        ABTI_xstream_unset_request(p_xstream, ABTI_XSTREAM_REQ_FREE_TIMER);
        ABTI_preemption_timer *p_ptimer = p_xstream->p_preemption_timer;
        ABTI_preemption_group *p_group = p_ptimer->p_group;
        ABTD_signal_timer_block(); /* TODO: unblock somewhere? */
...
int ABTI_xstream_schedule_thread(ABTI_xstream *p_xstream, ABTI_thread *p_thread)
{
    ...
    if (p_thread->is_sched != NULL) {
        ...
        /* set preemptiveness of this ES as a hint */
        if (p_thread->attr.preemption_type != ABT_PREEMPTION_DISABLED) {
            p_xstream->preemptive = ABT_TRUE;
        }

        ABTI_sub_xstream *p_sub_xstream = ABTI_sub_xstream_get_data();
        if (p_thread->preempted) {
            ABTI_PREEMPTION_DEBUG_1("switch to preempted thread (%p)", p_thread);
            /* Thread is preempted in a specific ES */
            p_xstream->p_wakeup_thread = p_thread;
            ABTI_thread_context_switch_sched_to_thread(p_sched,
                    p_sub_xstream->p_base_thread);
        } else {
            if (!p_sub_xstream->is_main && !p_sub_xstream->timer_started) {
                p_sub_xstream->timer_started = 1;

#if ABT_ENABLE_PREEMPTION_PROFILE
                uint64_t t1 = mlog_clock_gettime_in_nsec();
                ABTI_mlog_end_t(t1, "sched");
                ...
                if (gp_ABTI_global->preemption_timer_type == ABTI_PREEMPTION_TIMER_SIGNAL) {
                    ABTI_preemption_timer *p_ptimer = p_xstream->p_preemption_timer;
                    abt_errno =
                        ABTD_signal_timer_settime_abs(p_ptimer->offset,
                                p_ptimer->intvl, p_sub_xstream->timer);
                    ABTI_CHECK_ERROR(abt_errno);

#if ABT_ENABLE_PREEMPTION_PROFILE
                    uint64_t t2 = mlog_clock_gettime_in_nsec();
                    ABTI_mlog_end_t(t2, "set a timer");
                    ABTI_mlog_begin_t(t2);
#endif
                }
            }

            ABTI_PREEMPTION_DEBUG_1("switch to thread (%p)", p_thread);
        ...
        p_thread = ABTI_local_get_thread_explicit();

        ABTI_PREEMPTION_DEBUG_1("return from thread (%p)", p_thread);

        /* disable preemption of this ES from other ESes */
        p_xstream->preemptive = ABT_FALSE;

#if ABT_ENABLE_NORMAL_PROFILE
        ABTI_mlog_end_thread(ABTI_thread_get_id(p_thread));
        ABTI_mlog_begin();
#endif
#ifndef ABT_CONFIG_DISABLE_STACKABLE_SCHED
    }
...
void *ABTI_xstream_launch_main_sched(void *p_arg)
{
    ...
    p_sched->p_thread->is_sched = ABT_FALSE; /* TODO: fix */
    ABTI_sub_xstream_back_to_main(p_sched->p_thread);
...
static void ABTI_xstream_sleep_callback(void *arg) {
    ABTI_xstream *p_xstream = (ABTI_xstream *)arg;
    p_xstream->should_sleep = ABT_FALSE;
}
...
int ABTI_xstream_sleep() {
    ...
    abt_errno = ABTD_futex_sleep(&p_xstream->sleep_flag, ABTI_xstream_sleep_callback, (void *)p_xstream);
#else
    abt_errno = ABTD_signal_sleep(ABTI_xstream_sleep_callback, (void *)p_xstream);
#endif
    ABTI_CHECK_ERROR(abt_errno);
...
#if ABT_USE_FUTEX_ON_SLEEP
    abt_errno = ABTD_futex_wakeup(&p_xstream->sleep_flag);
#else
    abt_errno = ABTD_signal_wakeup(p_xstream->p_cur_sub->ctx);
#endif
    ABTI_CHECK_ERROR(abt_errno);
    ...
    while (found == ABT_FALSE) {
        ...
        for (i = 0; i < gp_ABTI_global->max_xstreams; i++) {
            if (gp_ABTI_global->p_xstreams[i] == NULL) {
                ...
                gp_ABTI_global->num_active_xstreams++;
                rank = i;
                found = ABT_TRUE;
                break;
            }
