...
int ABT_thread_set_preemption(ABT_thread thread, ABT_preemption_type type)
{
    ABTI_ENTER;
    int abt_errno = ABT_SUCCESS;
    ...
    p_attr->preemption_type = type;

    /* change preemptiveness of this ES if it is the local thread.
     * TODO: consider removing `thread` from parameters and changing only local
     * thread attributes */
    ABTI_thread *p_this_thread = ABTI_local_get_thread();
    if (p_this_thread == p_thread) {
        ABTI_xstream *p_xstream = ABTI_local_get_xstream();
        p_xstream->preemptive = (type != ABT_PREEMPTION_DISABLED);
    }
                       ...
                       ABT_bool push_pool, ABTI_thread **pp_newthread)
{
    ...
    p_newthread->preempted      = ABT_FALSE;
                                ...
                                ABTI_thread **pp_newthread)
{
    ...
    p_newthread->type           = ABTI_THREAD_TYPE_INTERNAL;
    p_newthread->p_req_arg      = NULL;
    p_newthread->p_keytable     = NULL;
    p_newthread->id             = ABTI_THREAD_INIT_ID;
    p_newthread->preempted      = ABT_FALSE;
