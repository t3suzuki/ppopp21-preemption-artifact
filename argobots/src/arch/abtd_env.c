...
void ABTD_env_init(ABTI_global *p_global)
{
    ...
    if (env != NULL) {
        if (strcasecmp(env, "signal") == 0) {
            p_global->preemption_timer_type = ABTI_PREEMPTION_TIMER_SIGNAL;
        } else if (strcasecmp(env, "dedicated") == 0) {
            p_global->preemption_timer_type = ABTI_PREEMPTION_TIMER_DEDICATED;
        }
    }

    /* Preemption timer interval in microseconds */
    env = getenv("ABT_PREEMPTION_INTERVAL_USEC");
    if (env == NULL) env = getenv("ABT_PREEMPTION_INTERVAL_USEC");
    if (env != NULL) {
        p_global->preemption_intvl_usec = atoi(env);
        ABTI_ASSERT(p_global->preemption_intvl_usec >= 1);
    } else {
        p_global->preemption_intvl_usec = 1000;
    }

    /* Initial # of sub ESs */
    env = getenv("ABT_INITIAL_NUM_SUB_XSTREAMS");
    if (env == NULL) env = getenv("ABT_INITIAL_NUM_SUB_XSTREAMS");
    if (env != NULL) {
        p_global->initial_num_sub_xstreams = atoi(env);
        ABTI_ASSERT(p_global->initial_num_sub_xstreams >= 1);
    } else {
